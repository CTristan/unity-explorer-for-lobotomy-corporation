...
{
    /// <summary>
    ///     Used to handle the underlying height data for a scroll pool, tracking which data values are at which position and how far they span.<br /><br /> A DataHeightCache is
    ///     created and managed automatically by a ScrollPool, you do not need to use this class yourself.
    /// </summary>
    public class DataHeightCache<T> where T : ICell
    {
        ...
        /// <summary>
        ///     Lookup table for "which data index first appears at this position"<br /> Index: DefaultHeight * index from top of data<br /> Value: the first data index at this position
        ///     <br />
        /// </summary>
        ...
        public float DefaultHeight =>
            m_defaultHeight ?? (float)(m_defaultHeight = ScrollPool.PrototypeHeight);
        ...
        public int GetFirstDataIndexAtPosition(float desiredHeight)
        {
            ...
            var rangeIndex = GetRangeFloorOfPosition(desiredHeight);
            ...
            {
                var idx = ScrollPool.DataSource.ItemCount - 1;

                return idx;
            }

            var dataIndex = rangeCache[rangeIndex];
            var cache = heightCache[dataIndex];

            // if the DataViewInfo is outdated, need to rebuild
            var expectedMin = GetRangeCeilingOfPosition(cache.startPosition);
            var expectedMax = expectedMin + cache.normalizedSpread - 1;
        ...
        /// <summary>
        ///     Get the spread of the height, starting from the start position.<br /><br /> The "spread" begins at the start of the next interval of the DefaultHeight, then increases for
        ///     every interval beyond that.
        /// </summary>
        private int GetRangeSpread(float startPosition,
            float height)
        {
            // get the remainder of the start position divided by min height
            var rem = startPosition % DefaultHeight;
        ...
        public void Add(float value)
        {
            ...
            var spread = GetRangeSpread(TotalHeight, value);

            heightCache.Add(new DataViewInfo(heightCache.Count, value, TotalHeight, spread));

            var dataIdx = heightCache.Count - 1;
            for (var i = 0; i < spread; i++) rangeCache.Add(dataIdx);
            ...
            float height)
        {
            ...
            var rangeIndex = GetRangeCeilingOfPosition(cache.startPosition);
            var spread = GetRangeSpread(cache.startPosition, height);
            ...
            {
                var spreadDiff = spread - cache.normalizedSpread;
        ...
        private void UpdateSpread(int dataIndex,
            int rangeIndex,
            int spreadDiff)
        {
            if (spreadDiff > 0)
        ...
        private void RecalculateStartPositions(int toIndex)
        {
            ...
            DataViewInfo cache;
            var prev = DataViewInfo.None;
            for (var idx = 0; idx <= toIndex && idx < heightCache.Count; idx++)
            {
                ...
                for (var i = 0; i < cache.normalizedSpread; i++) rangeCache.Add(cache.dataIndex);
    ...
    {
        // static
        public static DataViewInfo None { get; } = default;
        ...
        public override bool Equals(object obj)
        {
            ...
            return dataIndex == other.dataIndex && height == other.height && startPosition == other.startPosition && normalizedSpread == other.normalizedSpread;
        }
