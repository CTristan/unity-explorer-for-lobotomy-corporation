...
namespace UnityExplorerForLobotomyCorporation.UnityExplorer.UI.Panels
{
    public class CSConsolePanel : UEPanel
    {
        ...
        protected override void ConstructPanelContent()
        {
            ...
            var toolsRow = UIFactory.CreateHorizontalGroup(ContentRoot, "ToggleRow", false, false, true, true, 5, new Vector4(8, 8, 10, 5), default, TextAnchor.MiddleLeft);
            ...
            HelpDropdown.onValueChanged.AddListener(val =>
            {
                OnHelpDropdownChanged?.Invoke(val);
            });

            // Enable Ctrl+R toggle

            var ctrlRToggleObj = UIFactory.CreateToggle(toolsRow, "CtrlRToggle", out var CtrlRToggle, out var ctrlRToggleText);
            UIFactory.SetLayoutElement(ctrlRToggleObj, 150, flexibleWidth: 0, minHeight: 25);
            ctrlRToggleText.alignment = TextAnchor.UpperLeft;
            ctrlRToggleText.text = "Compile on Ctrl+R";
            CtrlRToggle.onValueChanged.AddListener(val =>
            {
                OnCtrlRToggled?.Invoke(val);
            });

            // Enable Suggestions toggle

            var suggestToggleObj = UIFactory.CreateToggle(toolsRow, "SuggestionToggle", out var SuggestionsToggle, out var suggestToggleText);
            UIFactory.SetLayoutElement(suggestToggleObj, 120, flexibleWidth: 0, minHeight: 25);
            suggestToggleText.alignment = TextAnchor.UpperLeft;
            suggestToggleText.text = "Suggestions";
            SuggestionsToggle.onValueChanged.AddListener(val =>
            {
                OnSuggestionsToggled?.Invoke(val);
            });

            // Enable Auto-indent toggle

            var autoIndentToggleObj = UIFactory.CreateToggle(toolsRow, "IndentToggle", out var AutoIndentToggle, out var autoIndentToggleText);
            UIFactory.SetLayoutElement(autoIndentToggleObj, 120, flexibleWidth: 0, minHeight: 25);
            autoIndentToggleText.alignment = TextAnchor.UpperLeft;
            autoIndentToggleText.text = "Auto-indent";
            AutoIndentToggle.onValueChanged.AddListener(val =>
            {
                OnAutoIndentToggled?.Invoke(val);
            });

            // Console Input

            var inputArea = UIFactory.CreateUIObject("InputGroup", ContentRoot);
            ...
            var inputObj = UIFactory.CreateScrollInputField(inputArea, "ConsoleInput", ConsoleController.STARTUP_TEXT, out var inputScroller, fontSize);
            ...
            inputScroller.Slider.Scrollbar.onValueChanged.AddListener(val =>
            {
                SetLinesPosition();
            });

            inputScroller.Slider.Slider.onValueChanged.AddListener(val =>
            {
                SetLinesPosition();
            });
