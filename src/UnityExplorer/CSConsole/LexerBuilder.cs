...
namespace UnityExplorerForLobotomyCorporation.UnityExplorer.CSConsole
{
    ...
    public class LexerBuilder
    {
        ...
        public LexerBuilder()
        {
            lexers = new Lexer[]
            {
                commentLexer, stringLexer, new SymbolLexer(), new NumberLexer(), new KeywordLexer(),
            };

            foreach (var matcher in lexers)
            {
                foreach (var c in matcher.Delimiters)
                {
        ...
        /// <summary>Parse the range of the string with the Lexer and build a RichText-highlighted representation of it.</summary>
        ...
        public string BuildHighlightedString(string input,
            int startIdx,
            int endIdx,
            int leadingLines,
            int caretIdx,
            out bool caretInStringOrComment)
        {
            ...
            for (var i = 0; i < leadingLines; i++) sb.Append('\n');

            var lastUnhighlighted = startIdx;
            foreach (var match in GetMatches())
            {
                // append non-highlighted text between last match and this
                for (var i = lastUnhighlighted; i < match.startIndex; i++) sb.Append(input[i]);

                // append the highlighted match
                sb.Append(match.htmlColorTag);
                for (var i = match.startIndex; i <= match.endIndex && i <= currentEndIdx; i++) sb.Append(input[i]);
                sb.Append(SignatureHighlighter.CLOSE_COLOR);

                // update the last unhighlighted start index
                lastUnhighlighted = match.endIndex + 1;

                var matchEndIdx = match.endIndex;
        ...
        public IEnumerable<MatchInfo> GetMatches()
        {
            ...
            while (!EndOfInput)
            {
                SkipWhitespace();
                var anyMatch = false;
                var startIndex = CommittedIndex + 1;
        ...
        public bool IsDelimiter(char character,
            bool orWhitespace = false,
            bool orLetterOrDigit = false)
        {
            return delimiters.Contains(character) || (orWhitespace && char.IsWhiteSpace(character)) || (orLetterOrDigit && char.IsLetterOrDigit(character));
        }
        ...
        public string IndentCharacter(string input,
            ref int caretIndex)
        {
            var lastCharIndex = caretIndex - 1;
            var c = input[lastCharIndex];
        ...
        private string IndentNewLine(string input,
            int indent,
            ref int caretIndex)
        {
            ...
            while (CurrentIndex < input.Length - 1)
            {
                ...
                if (IndentCloseChars.Contains(next))
                {
                    indent--;
                }
        ...
        private string IndentCloseDelimiter(string input,
            int indent,
            int lastCharIndex,
            ref int caretIndex)
        {
