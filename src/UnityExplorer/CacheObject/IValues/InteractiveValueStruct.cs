...
namespace UnityExplorerForLobotomyCorporation.UnityExplorer.CacheObject.IValues
{
    public class InteractiveValueStruct : InteractiveValue
    {
        ...
        public override void SetValue(object value)
        {
            ...
            for (var i = 0; i < CurrentInfo.Fields.Length; i++) inputFields[i].Text = CurrentInfo.GetValue(RefInstance, i);
        }
        ...
        private void OnApplyClicked()
        {
            try
            {
                for (var i = 0; i < CurrentInfo.Fields.Length; i++) CurrentInfo.SetValue(RefInstance, inputFields[i].Text, i);
        ...
        {
            for (var i = 0; i < CurrentInfo.Fields.Length || i <= inputFields.Count; i++)
            {
                ...
                var label = SignatureHighlighter.Parse(CurrentInfo.Fields[i].FieldType, false);
        ...
        private void AddEditorRow()
        {
            ...
            var label = UIFactory.CreateLabel(row, "Label", "notset");
        ...
        {
            UIRoot = UIFactory.CreateVerticalGroup(parent, "InteractiveValueStruct", false, false, true, true, 3, new Vector4(4, 4, 4, 4), new Color(0.06f, 0.06f, 0.06f), TextAnchor.MiddleLeft);
        ...
        public class StructInfo
        {
            ...
            public StructInfo(bool isSupported,
                FieldInfo[] fields)
            {
                ...
                int fieldIndex)
            {
                ...
                {
                    if (!ParseUtility.TryParse(input, field.FieldType, out val, out var ex))
                    {
