...
namespace UnityExplorerForLobotomyCorporation.UnityExplorer.ObjectExplorer
{
    public class SceneExplorer : UIModel
    {
        ...
        /// <summary>Whether to automatically update per auto-update interval or not.</summary>
        ...
        private IEnumerable<GameObject> GetRootEntries()
        {
            return SceneHandler.CurrentRootObjects;
        }
        ...
        public void JumpToTransform(Transform transform)
        {
            ...
            UIManager.SetPanelActive(Parent, true);
            Parent.SetTab(0);
        ...
        private void SceneHandler_OnInspectedSceneChanged(Scene scene)
        {
            ...
            if (sceneToDropdownOption.ContainsKey(scene))
            {
                ...
                var idx = sceneDropdown.options.IndexOf(opt);
        ...
        private void TryLoadScene(LoadSceneMode mode,
            Dropdown allSceneDrop)
        {
        ...
        public override void ConstructUI(GameObject content)
        {
            ...
            var toolbar = UIFactory.CreateVerticalGroup(uiRoot, "Toolbar", true, true, true, true, 2, new Vector4(2, 2, 2, 2), new Color(0.15f, 0.15f, 0.15f));
            ...
            RuntimeHelper.SetColorBlock(inputField.Component, new Color(0.4f, 0.4f, 0.4f), new Color(0.2f, 0.2f, 0.2f), new Color(0.08f, 0.08f, 0.08f));
            ...
            UIFactory.SetLayoutElement(refreshButton.Component.gameObject, 65, flexibleWidth: 0);
            refreshButton.OnClick += UpdateTree;

            var refreshToggle = UIFactory.CreateToggle(refreshRow, "RefreshToggle", out var toggle, out var text);
            ...
            text.fontSize = 12;
            toggle.isOn = false;
            toggle.onValueChanged.AddListener(val => AutoUpdate = val);
            ...
            var nameLabel = UIFactory.CreateLabel(labelsRow, "NameLabel", "Name", color: Color.grey);
            UIFactory.SetLayoutElement(nameLabel.gameObject, flexibleWidth: 9999, minHeight: 25);

            var indexLabel = UIFactory.CreateLabel(labelsRow, "IndexLabel", "Sibling Index", fontSize: 12, color: Color.grey);
            UIFactory.SetLayoutElement(indexLabel.gameObject, 100, flexibleWidth: 0, minHeight: 25);

            // Transform Tree

            var scrollPool = UIFactory.CreateScrollPool<TransformCell>(uiRoot, "TransformTree", out var scrollObj, out var scrollContent, new Color(0.11f, 0.11f, 0.11f));
        ...
        private void OnCellClicked(GameObject obj)
        {
        ...
        private void ConstructSceneLoader()
        {
            ...
            try
            {
                if (SceneHandler.WasAbleToGetScenesInBuild)
                {
                    ...
                    var loaderTitle = UIFactory.CreateLabel(sceneLoaderObj, "SceneLoaderLabel", "Scene Loader", TextAnchor.MiddleLeft, Color.white);
                    ...
                    allSceneDropdown.onValueChanged.AddListener(val =>
                    {
                        RefreshSceneLoaderButtons();
                    });
                }
