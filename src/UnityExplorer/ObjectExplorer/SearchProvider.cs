...
namespace UnityExplorerForLobotomyCorporation.UnityExplorer.ObjectExplorer
{
    ...
    public static class SearchProvider
    {
        ...
        {
            "m_instance", "m_Instance", "s_instance", "s_Instance", "_instance", "_Instance", "instance", "Instance", "<Instance>k__BackingField", "<instance>k__BackingField",
        };

        private static bool Filter(Scene scene,
            SceneFilter filter)
        {
            if (filter == SceneFilter.Any)
            {
                return true;
            }

            if (filter == SceneFilter.DontDestroyOnLoad)
            {
                return scene.handle == -12;
            }

            if (filter == SceneFilter.HideAndDontSave)
            {
                return scene == default;
            }

            if (filter == SceneFilter.ActivelyLoaded)
            {
        ...
        internal static List<object> UnityObjectSearch(string input,
            ...
            SceneFilter sceneFilter)
        {
            ...
            if (!string.IsNullOrEmpty(customTypeInput))
            {
                ...
                {
                    if (typeof(Object).IsAssignableFrom(customType))
                    {
            ...
            var shouldFilterGOs = searchType == typeof(GameObject) || typeof(Component).IsAssignableFrom(searchType);
                ...
                if (go)
                {
                    ...
                    if (shouldFilterGOs)
                    {
                        ...
                        if (childFilter != ChildFilter.Any)
                        {
                            ...
                            if (childFilter == ChildFilter.RootObject && go.transform.parent)
                            {
                                continue;
                            }
                        }
        ...
        internal static List<object> InstanceSearch(string input)
        {
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                foreach (var type in asm.GetTypes().Where(it => !(it.IsSealed && it.IsAbstract) && !it.IsEnum))
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(nameFilter) && !type.FullName.ContainsIgnoreCase(nameFilter))
