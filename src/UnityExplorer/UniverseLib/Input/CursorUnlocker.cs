...
{
    /// <summary>Handles taking control of the mouse/cursor and EventSystem (depending on Config settings) when a UniversalUI is being used.</summary>
    public class CursorUnlocker
    {
        ...
        /// <summary>True if a UI is being displayed and <see cref="ConfigManager.Force_Unlock_Mouse" /> is true.</summary>
        public static bool ShouldUnlock =>
            ConfigManager.Force_Unlock_Mouse && UniversalUI.AnyUIShowing;

        [Obsolete("Moved to EventSystemHelper")]
        public static EventSystem CurrentEventSystem =>
            EventSystemHelper.CurrentEventSystem;
        ...
        /// <summary>Uses WaitForEndOfFrame in a Coroutine to aggressively set the Cursor state every frame.</summary>
        ...
        /// <summary>Checks current ShouldUnlock state and sets the Cursor and EventSystem as required.</summary>
        ...
        {
            Universe.Patch(typeof(Cursor), "lockState", MethodType.Setter, prefix: AccessTools.Method(typeof(CursorUnlocker), nameof(Prefix_set_lockState)));

            Universe.Patch(typeof(Cursor), "visible", MethodType.Setter, prefix: AccessTools.Method(typeof(CursorUnlocker), nameof(Prefix_set_visible)));
        }
