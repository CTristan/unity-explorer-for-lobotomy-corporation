...
{
    /// <summary>Handles updating, dragging and resizing all <see cref="PanelBase" />s for the parent <see cref="UIBase" />.</summary>
    public class PanelManager
    {
        ...
        /// <summary>Determines if the PanelManager should update "focus" (ie. heirarchy order). By default, returns true if user is clicking.</summary>
        protected virtual bool ShouldUpdateFocus =>
            MouseInTargetDisplay && (InputManager.GetMouseButtonDown(0) || InputManager.GetMouseButtonDown(1));

        /// <summary>The MousePosition which should be used for this PanelManager. By default, this is <see cref="InputManager.MousePosition" />.</summary>
        protected internal virtual Vector3 MousePosition =>
            InputManager.MousePosition;

        /// <summary>The Screen dimensions which should be used for this PanelManager. By default, this is <see cref="Screen.width" /> x <see cref="Screen.height" />.</summary>
        protected internal virtual Vector2 ScreenDimensions =>
            new Vector2(Screen.width, Screen.height);

        /// <summary>Determines if the mouse is currently in the Display used by this PanelManager. By default, this always returns true.</summary>
        protected virtual bool MouseInTargetDisplay =>
            true;
        ...
        protected virtual void UpdateFocus()
        {
            ...
            if (!focusHandledThisFrame)
            {
                ...
                var count = PanelHolder.transform.childCount;

                for (var i = count - 1; i >= 0; i--)
                {
                    // make sure this is a real recognized panel
                    var transform = PanelHolder.transform.GetChild(i);
                    if (!transformIDToUIPanel.TryGetValue(transform.GetInstanceID(), out var panel))
                    {
                        continue;
                    }
        ...
        {
            draggerInstances.Sort((a,
                b) => b.Rect.GetSiblingIndex().CompareTo(a.Rect.GetSiblingIndex()));
        }

        /// <summary>Updates all PanelDraggers owned by this PanelManager.</summary>
        protected internal virtual void UpdateDraggers()
        {
            ...
            foreach (var instance in draggerInstances)
            {
        ...
        public static bool ResizePrompting =>
            resizeCursor && resizeCursorUIBase.Enabled;
        ...
        public static void ForceEndResize()
        {
            ...
            foreach (var instance in allDraggers)
            {
                instance.WasDragging = false;
        ...
        protected static void CreateResizeCursorUI()
        {
            try
            {
                ...
                var parent = resizeCursorUIBase.RootObject;
                parent.transform.SetParent(UniversalUI.CanvasRoot.transform);

                var text = UIFactory.CreateLabel(parent, "ResizeCursor", "↔", TextAnchor.MiddleCenter, Color.white, true, 35);
                ...
                var rect = resizeCursor.GetComponent<RectTransform>();
