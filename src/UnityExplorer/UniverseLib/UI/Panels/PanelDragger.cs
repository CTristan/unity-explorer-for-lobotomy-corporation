...
namespace UnityExplorerForLobotomyCorporation.UniverseLib.UI.Panels
{
    public class PanelDragger
    {
        ...
        public PanelBase UIPanel { get; }

        public bool AllowDragAndResize =>
            UIPanel.CanDragAndResize;
        ...
        protected internal virtual void Update(MouseState state,
            Vector3 rawMousePos)
        {
            ResizeTypes type;
            var resizePos = Rect.InverseTransformPoint(rawMousePos);
            var inResizePos = MouseInResizeArea(resizePos);

            var dragPos = DragableArea.InverseTransformPoint(rawMousePos);
            var inDragPos = DragableArea.rect.Contains(dragPos);
        ...
        {
            var mousePos = UIPanel.Owner.Panels.MousePosition;

            var diff = (Vector2)mousePos - lastDragPosition;
        ...
        {
            totalResizeRect = new Rect(Rect.rect.x - RESIZE_THICKNESS + 1, Rect.rect.y - RESIZE_THICKNESS + 1, Rect.rect.width + DBL_THICKESS - 2, Rect.rect.height + DBL_THICKESS - 2);

            // calculate the four cross sections to use as flags
            if (AllowDragAndResize)
            {
                m_resizeMask[ResizeTypes.Bottom] = new Rect(totalResizeRect.x, totalResizeRect.y, totalResizeRect.width, RESIZE_THICKNESS);

                m_resizeMask[ResizeTypes.Left] = new Rect(totalResizeRect.x, totalResizeRect.y, RESIZE_THICKNESS, totalResizeRect.height);

                m_resizeMask[ResizeTypes.Top] = new Rect(totalResizeRect.x, Rect.rect.y + Rect.rect.height - 2, totalResizeRect.width, RESIZE_THICKNESS);

                m_resizeMask[ResizeTypes.Right] = new Rect(totalResizeRect.x + Rect.rect.width + RESIZE_THICKNESS - 2, totalResizeRect.y, RESIZE_THICKNESS, totalResizeRect.height);
            }
        }
        ...
        public virtual void OnHoverResize(ResizeTypes resizeType)
        {
            ...
            var rot = PanelManager.resizeCursor.transform.rotation;
        ...
        {
            var mousePos = UIPanel.Owner.Panels.MousePosition;
            var rect = UIPanel.Owner.RootRect;
        ...
        {
            var mousePos = UIPanel.Owner.Panels.MousePosition;
            var diff = lastResizePos - (Vector2)mousePos;
            ...
            var screenDimensions = UIPanel.Owner.Panels.ScreenDimensions;
            ...
            var diffX = (float)((decimal)diff.x / (decimal)screenDimensions.x);
            var diffY = (float)((decimal)diff.y / (decimal)screenDimensions.y);
            ...
            var prevMin = Rect.anchorMin;
            var prevMax = Rect.anchorMax;
