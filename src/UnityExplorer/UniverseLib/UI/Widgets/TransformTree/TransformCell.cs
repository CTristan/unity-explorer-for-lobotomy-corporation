...
namespace UnityExplorerForLobotomyCorporation.UniverseLib.UI.Widgets.TransformTree
{
    public class TransformCell : ICell
    {
        ...
        public GameObject CreateContent(GameObject parent)
        {
            ...
            UIFactory.SetLayoutElement(spacerObj, 0, flexibleWidth: 0, minHeight: 0, flexibleHeight: 0);
            spacer = spacerObj.GetComponent<LayoutElement>();

            // Expand arrow

            ExpandButton = UIFactory.CreateButton(UIRoot, "ExpandButton", "►");
            UIFactory.SetLayoutElement(ExpandButton.Component.gameObject, 15, flexibleWidth: 0, minHeight: 25, flexibleHeight: 0);
            ...
            var nameBtnHolder = UIFactory.CreateHorizontalGroup(UIRoot, "NameButtonHolder", false, false, true, true, childAlignment: TextAnchor.MiddleLeft);
            ...
            SiblingIndex = UIFactory.CreateInputField(UIRoot, "SiblingIndexInput", string.Empty);
            ...
            siblingImage.color = new Color(0f, 0f, 0f, 0.25f);
            ...
            var highlight = new Color(0.25f, 0.25f, 0.25f);
        ...
        public void ConfigureCell(CachedTransform cached)
        {
            ...
            if (cached.Value)
            {
                var name = cached.Value.name?.Trim();
                ...
                var childCount = cached.Value.childCount;
                if (childCount > 0)
                {
        ...
        {
            if (cachedTransform == null || !cachedTransform.Value)
            {
                return;
            }
            ...
            {
                cachedTransform.Value.SetSiblingIndex(index);
            }

            SiblingIndex.Text = cachedTransform.Value.GetSiblingIndex().ToString();
        }
    }
}
