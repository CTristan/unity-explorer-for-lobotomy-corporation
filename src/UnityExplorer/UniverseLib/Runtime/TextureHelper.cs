...
{
    /// <summary>Helper class for working with <see cref="Texture2D" />s, including some runtime-specific helpers and some general utility.</summary>
    public abstract class TextureHelper
    {
        ...
        /// <summary>Returns true if it is possible to force-read non-readable Cubemaps in this game.</summary>
        public static bool CanForceReadCubemaps =>
            Instance.Internal_CanForceReadCubemaps;

        internal abstract bool Internal_CanForceReadCubemaps { get; }

        /// <summary>Helper for invoking Unity's <c>ImageConversion.EncodeToPNG</c> method.</summary>
        public static byte[] EncodeToPNG(Texture2D tex)
        {
            return Instance.Internal_EncodeToPNG(tex);
        }

        protected internal abstract byte[] Internal_EncodeToPNG(Texture2D tex);

        /// <summary>Helper for creating a new <see cref="Texture2D" />.</summary>
        public static Texture2D NewTexture2D(int width,
            int height)
        {
            return Instance.Internal_NewTexture2D(width, height);
        }
        ...
        public static Texture2D NewTexture2D(int width,
            ...
            bool mipChain)
        {
            return Instance.Internal_NewTexture2D(width, height, textureFormat, mipChain);
        }

        protected internal abstract Texture2D Internal_NewTexture2D(int width,
            int height);
        ...
        /// <summary>Helper for calling Unity's <see cref="Graphics.Blit" /> method.</summary>
        public static void Blit(Texture tex,
            RenderTexture rt)
        {
            Instance.Internal_Blit(tex, rt);
        }

        protected internal abstract void Internal_Blit(Texture tex,
            RenderTexture rt);

        /// <summary>Helper for creating a <see cref="Sprite" /> from the provided <paramref name="texture" />.</summary>
        public static Sprite CreateSprite(Texture2D texture)
        {
            return Instance.Internal_CreateSprite(texture);
        }

        protected internal abstract Sprite Internal_CreateSprite(Texture2D texture);

        /// <summary>Helper for creating a <see cref="Sprite" /> from the provided <paramref name="texture" />.</summary>
        public static void CreateSprite(Texture2D texture,
            Rect rect,
            Vector2 pivot,
            float pixelsPerUnit,
            uint extrude,
        ...
        /// <summary>Helper for checking <c>Texture2D.isReadable</c>.</summary>
        ...
        /// <summary>Helper for checking <c>Cubemap.isReadable</c>.</summary>
        ...
        /// <summary>
        ///     Copies the provided <paramref name="source" /> into a readable <see cref="Texture2D" />. <br /> Supports <see cref="Texture2D" /> and individual <see cref="Cubemap" />
        ///     faces.
        /// </summary>
            ...
            int dstY = 0)
        {
            ...
            var newTex = NewTexture2D((int)dimensions.width, (int)dimensions.height, format, false);
        ...
        /// <summary>
        ///     Copies the provided <paramref name="source" /> into the <paramref name="destination" /> Texture. <br /><br />Supports <see cref="Texture2D" /> and individual
        ///     <see cref="Cubemap" /> faces.
        /// </summary>
        /// <param name="source">The original Texture to copy from.</param>
            ...
            int dstY = 0)
        {
            try
            {
                ...
                {
                    dimensions = new Rect(0, 0, source.width, source.height);
                }
        ...
        internal abstract Texture Internal_CopyTexture(Texture src,
            int srcElement,
            int srcMip,
            int srcX,
            int srcY,
        ...
        /// <summary>
        ///     Unwraps the Cubemap into a Texture2D, showing the the X faces on the left, Y in the middle and Z on the right, with positive faces on the top row and negative on the
        ///     bottom.
        /// </summary>
        public static Texture2D UnwrapCubemap(Cubemap cubemap)
        {
            ...
            var newTex = NewTexture2D(cubemap.width * 3, cubemap.height * 2, cubemap.format, false);

            for (var i = 0; i < 6; i++)
            {
                var x = i % 3 * cubemap.width;
                var y = i % 2 * cubemap.height;
        ...
        /// <summary>Converts the <paramref name="origTex" /> into a readable <see cref="TextureFormat.ARGB32" />-format <see cref="Texture2D" />. <br /><br />Supports non-readable Textures.</summary>
        public static Texture2D CopyToARGB32(Texture2D origTex,
            Rect dimensions = default,
            int dstX = 0,
            int dstY = 0)
        {
            ...
            if (dimensions == default)
            {
                dimensions = new Rect(0, 0, origTex.width, origTex.height);
            }

            var origFilter = origTex.filterMode;
        ...
        /// <summary>
        ///     Saves the provided <paramref name="tex" /> as a PNG file, into the provided <paramref name="dir" /> as "<paramref name="name" />.png". <br /><br />To save a
        ///     <see cref="Cubemap" />, use <see cref="Copy" /> for each face, using the <c>cubemapFace</c> parameter to select the face.
        /// </summary>
        public static void SaveTextureAsPNG(Texture2D tex,
            string dir,
            string name)
        {
        ...
        /// <summary>
        ///     Saves the provided <paramref name="texture" /> as a PNG file to the provided path. <br /><br />To save a <see cref="Cubemap" />, use <see cref="Copy" /> for each face,
            ...
            string fullPathWithFilename)
        {
            ...
            var data = Instance.Internal_EncodeToPNG(tex);
            ...
            var dir = Path.GetDirectoryName(fullPathWithFilename);
            var name = Path.GetFileNameWithoutExtension(fullPathWithFilename);
        ...
        public static Texture2D ForceReadTexture(Texture2D origTex,
            Rect dimensions = default)
        {
            return Copy(origTex, dimensions);
        }

        [Obsolete("Use TextureHelper.CopyTexture() instead. This method will be removed in a future version of UniverseLib.")]
        public static Texture2D Copy(Texture2D orig)
        {
            return Copy(orig, new Rect(0, 0, orig.width, orig.height));
        }
